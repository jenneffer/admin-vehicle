...
$query = "SELECT si.id, si.item_name, SUM(quantity) AS stock_out, (SELECT SUM(stock_in) 
        FROM om_stock WHERE item_id = si.id AND date_added BETWEEN '".dateFormat($date_start)."' AND '".dateFormat($date_end)."' ) AS stock_in,         
        si.unit 
        FROM om_stock_item si
        LEFT JOIN om_stock_take sst ON sst.item_id = si.id AND sst.date_taken BETWEEN '".dateFormat($date_start)."' AND '".dateFormat($date_end)."'
        GROUP BY si.id
...
//                                         var_dump($data);
                                        //get the balance from om_stock_balance table
                                        $balance = itemName("SELECT stock_balance FROM om_stock_balance WHERE item_id='".$data['id']."'");
//                                         $balance = $data['stock_in'] - $data['stock_out'];
